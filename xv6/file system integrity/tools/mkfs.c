...
iappend(uint inum, void *xp, int n)
{
  ...
  while(n > 0){
    ...
    if(fbn < NDIRECT){
      ...
      if(xint(din.indir_addr) == 0){
        // printf("allocate indirect block\n");
        din.indir_addr = xint(freeblock++);
        usedblocks++;
      }
      // printf("read indirect block\n");
      rsect(xint(din.indir_addr), (char*)indirect);
      if(indirect[2 *(fbn - NDIRECT)] == 0){
        indirect[2 * (fbn - NDIRECT)] = xint(freeblock++);
        usedblocks++;
        wsect(xint(din.indir_addr), (char*)indirect);
      }
      x = xint(indirect[2 * (fbn-NDIRECT)]);
    }
    ...
    //    ** generate checksums **
    uint csm = adler32((void *)buf, 512);

    if(fbn < NDIRECT)
      din.checksum[fbn] = xint(csm);
    else
    {
      indirect[2 * (fbn-NDIRECT) + 1] = xint(csm);
      wsect(xint(din.indir_addr), (char *)indirect);
    }
